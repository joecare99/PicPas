...
begin
  ...
  stConst_Expres: begin  //la expresión p2 se evaluó y esta en W
    SetROBResultExpres_byte(Opt);
    aux := GetAuxRegisterByte;
    kMOVWF(aux.addr);   //guarda temporalmente en aux
    kMOVLW(value1);
    kADDWF(aux.addr, toW);  //deja en W
    aux.used := false;
  end;
...
begin
  ...
  stConst_Variab: begin
    ...
    aux := GetAuxRegisterByte;
    kCLRF(aux.addr);  //Usa temporalmente
    kSUBWF(aux.addr, toW);  //Invierte W: 0 - W -> W
    aux.used := false;
  end;
  stConst_Expres: begin  //la expresión p2 se evaluó y esta en W
    SetROBResultExpres_byte(Opt);
    ...
    kMOVWF(aux.addr);
    kMOVLW(value1);
    kSUBWF(aux.addr, toW);   //aux - value1 -> W
    kCLRF(aux.addr);  //Usa temporalmente
    kSUBWF(aux.addr, toW);  //Invierte W: 0 - W -> W
    aux.used := false;
  end;
  ...
  stExpres_Const: begin   //la expresión p1 se evaluó y esta en W
    ...
    kMOVWF(aux.addr);
    kMOVLW(value2);
    kSUBWF(aux.addr, toW);   //W - value2 -> value2
    aux.used := false;
  end;
  stExpres_Variab:begin  //la expresión p1 se evaluó y esta en W
    SetROBResultExpres_byte(Opt);
...
//      GenWarn('Expression will always be FALSE.');  //o TRUE si la lógica Está invertida
      ...
      aux := GetAuxRegisterByte;
      kMOVWF(aux.addr);   //Guarda W temporalmente
  ...
  stVariab_Const: begin
    if value2 = 255 then begin
      ...
      kMOVF(byte1, toW);
      kSUBWF(aux.addr, toW);  //Si p1 > p2: C=0.
      CopyInvert_C_to_Z; //Pasa C a Z (invirtiendo)
      aux.used := false;
    end;
  end;
...
begin
  ...
  loop1 := _PC;
  kMOVF(counter.addr, toW);  //¿Igual a 0?
...
_LABEL(sale); //Si p1=p2 -> Z=1. Si p1>p2 -> C=0.
  ...
  stExpres_Const: begin   //la expresión p1 se evaluó y esta en H,W
    ...
    _MOVWF(E.offs);  //guarda W
    varTmp := NewTmpVarWord(E, H);  //Crea variable temporal
...
begin
  ...
  stExpres_Const: begin   //la expresión p1 se evaluó y esta en (H,W)
    ...
    kMOVWF(L.addr);  //Salva W
    kMOVLW(value2);
    kADDWF(L.addr, toW);   //Suma byte de menor peso
    _BANKSEL(H.bank);      //se cambia primero el banco, por si acaso.
    _btfsc(_STATUS,_C);    //Hubo acarreo anterior?
    _incf(H.offs, toF);
    L.used := false;
...
MUL16LOOP := _PC;
   ...
   kMOVWF(aux.addr);      //Salva W por si necesita incementarse. No cambia banderas.
   _BTFSC   (_STATUS,0  );
...
begin
  ...
  stConst : begin
    ...
    if value2 = 0 then begin
      ...
      kADDWF (byte1L, toF);
      kMOVLW (value2H);
      kMOVWF(aux.addr);  //Salva sin cambiar C
      _btfsc (_STATUS,_C);
      _INCF(aux.offs, toW); //W=W+1. El banco aquí, debe ser el de "aux".
      kADDWF (byte1H, toF);
      kMOVLW (p2^.EByte);
...
begin
  ...
  if HayError then exit;
  aux := GetAuxRegisterByte;  //Pide un registro libre (En esta familia no se puede usar FSR, porque no está implementado con todos sus bits.)
